...
contract UniswapV2PairTest is Test {
    ...
    UniswapV2Pair pair;
    UserForTest userTest;

    function setUp() public {
        userTest = new UserForTest();
        ...
        token0.mint(10 ether, address(userTest));
        token1.mint(10 ether, address(userTest));
    }
    ...
    function testMintWithLiquidityExists() public {
        ...
        vm.warp(37); // Advances the virtual machine's block timestamp by 37 seconds. This is done to simulate the passage of time.
    ...
    function testMintWithUnderflowLiquidity() public {
        ...
        vm.expectRevert(ErrorInEncode("Panic(uint256)", 0x11)); //used to expect that the ErrorInEncode() function with the signature "Panic(uint256)" will revert the transaction with a specific error code 0x11 which represents an arithmetic operation resulting in an underflow outside of an unchecked block.
        pair.mint(address(this));
    }
    ...
    function testMintWithLiquidityZero() public {
        ...
        vm.expectRevert(ErrorInEncode("InsufficientLiquidityMinted()"));
        pair.mint(address(this));
    }
    ...
    function testSwap() public {
        ...
        assertEq(
            ...
            "token0 balance is not correct"
        );
        assertEq(
            token1.balanceOf(address(this)),
            10 ether - 2 ether + amountOut,
            "token1 balance is not correct"
        );
    ...
    function testSwapZeroOut() public {
        ...
        vm.expectRevert(ErrorInEncode("InsufficientOutputAmount()"));
